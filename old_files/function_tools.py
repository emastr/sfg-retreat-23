...
def get_sol_analytic(u0: Callable[[MAT, MAT], MAT], w: Callable[[MAT, MAT], MAT], T) -> Callable[[MAT, MAT], MAT]:
    ...
    Args:
        u0 (Callable[[MAT, MAT], MAT]): _description_
...
def load_end_state(path=None, K=129):
    # Load image
    img = np.sum(im.imread(path), axis=2).astype(float)/255
    img = img[:-1, :-1]
